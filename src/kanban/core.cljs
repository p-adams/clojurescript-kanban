...
(defonce backlog-state (reagent/atom {1 {:id 1 :task "create RESTFUL endpoints" :completed false :owner "Mary Smith"}
...
(defn move-activity [activity activities destination]
  ...
  (let [id (get (val activity) :id)
        ...
        owner (get (val activity) :owner)]
  (swap! destination assoc id {:id id :task task :completed completed :owner owner}))
  (remove-activity (get (val activity) :id) activities))
...
(defn add-task-to-backlog [task owner]
  "adds new task to backlog and resets the inputs"
